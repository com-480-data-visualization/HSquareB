...
The first was the EDA. The numbers kept arriving in ways that a two-mode toggle would have buried. Each finding pointed in a different direction, and leaving the reader to discover all of them by switching colour modes assumed both domain knowledge and patience that most readers would not have.
...
**The Italy divide.** Over 18 months, Germany recorded 846 hours of negative prices (6.5% of all hours), France 715 (5.5%), Austria 584 (4.5%), Switzerland 529 (4.0%). Northern Italy: zero. Not a handful, but zero across the entire window. Italy sits behind the Alps, relies heavily on gas-fired generation, and is two interconnectors further from Germany's solar core. That divide became the narrative's counterpoint: Italy as the insulated market that makes Switzerland's 529 hours feel structural rather than accidental.

![Negative-price hours by country across 18 months. The Italy versus rest-of-Europe divide (zero versus hundreds) became the counterpoint that anchors the story.](../milestone_2/figures/eda_neg_hours.png)
...
The EDA left us with findings that were individually striking and collectively coherent, but they needed to be narrated, not just displayed. A reader who arrives at a choropleth map without context will see colours change but will not know what they mean.
...
| 6 | The duck deepens | Animated monthly price profiles, Jan 2024 to Jun 2025 |
...
Step 3 went through one significant revision. The original sketch showed a full-day price line chart. After building both versions, we set them side by side: the line chart required the reader to scan a curve and locate the minimum, whereas the single number `−€145.12` at typographic scale landed immediately. We dropped the line chart entirely, promoted the number to a full-screen bloom, and gave Germany's simultaneous price (`−€135.45`) in a supporting sentence for scale.
...
The site is dark-only. The background uses a three-level navy ramp (`#0a0e1a` to `#131827` to `#1c2235`).
...
**Typography.** Fraunces (variable serif) for editorial copy and hero numbers. JetBrains Mono for every data value, label, country code, and legend tick. Numbers are never set in the serif. The pairing gives the body editorial warmth and the data machine precision.

**Flow arrows.** Width is proportional to the absolute price spread between bidding zones, direction is inferred from the differential (electricity flows from cheap to expensive), and arrows are drawn as quadratic bezier curves whose control points are offset to the left of travel at 18 percent of the segment length, so adjacent flows (for example CH to IT and AT to IT) do not overlap. Endpoints are inset by 28 pixels so the arrows do not collide with the country price labels. We do not have net transfer data, so directional inference is disclosed in the footer.
...
No framework, no build step. Every ES import is resolved from a browser importmap. Avoiding a bundler kept the dev loop instant (edit, then refresh) and removed an entire class of dependency-resolution bugs.
...
The calendar heatmap uses a Canvas renderer rather than SVG. The first SVG implementation took about 340 ms to render and caused layout thrash each time the DE/CH tab toggle fired. Moving to Canvas brought repaint time to about 4 ms via `ctx.clearRect` plus synchronous redraw. The Canvas element carries an `aria-description` so screen readers still receive the data summary.
...
**Ice-white for the most negative prices.** We tested three colour scales before settling on the current one. A red-to-blue diverging scale made negative prices look "cold", which is semantically misleading since they represent economic stress. A green-negative, red-positive scale violated traffic-light conventions. The ice-white to navy to orange scale was the only one where Switzerland going lighter than Germany at Step 3 was immediately visible on the dark background without any annotation pointing to it.

**Adding a "Both" mode instead of forcing a choice.** The original explorer offered two colour modes: Price or Renewable. Toggling between them made the correlation between solar output and price collapse visible but effortful, because the reader had to hold one map in memory while looking at the other. We added a third mode, Both, that keeps the price colour scale on the country fills while pseudo-3D bars rise beside each label to show renewable output in GW. A square-root height scale ensures smaller producers like Switzerland and Austria remain legible next to Germany's much larger output. Two legends appear simultaneously, one for each encoding. The mode was the last major feature added and came directly from noticing that neither Price nor Renewable alone told the causal story as clearly as both together.

---

## 7. Peer assessment

All three of us contributed across every stage of the project. The framing of the question, the choice of dataset, the move from a single choropleth to a guided narrative followed by an explorer, the seven-step arc, the visual language, and the final round of edits were decisions we made together through repeated team discussion. The notes below describe where each of us focused, but every part of the piece carries traces of all three.

**Brian Banna** worked on the engineering side. He built the Python preprocessing pipeline that converts the raw ENTSO-E CSV into the JSON the site uses, the D3 map module with its flow-arrow layer and live price labels, the seven-step scroll orchestration on Scrollama, and the interactive explorer with its timeline scrubber and country sidebar. He put together the visual design system (the navy palette, the ice-white-to-orange price scale, the Fraunces and JetBrains Mono typography pairing) and ran the mobile and accessibility passes.

**Lê Thào Huyèn** worked on the editorial side. She shaped the seven-step narrative arc, wrote the step copy including the Step 3 hero framing and the Italy closer, and after Milestone 2 added the Both mode to the explorer, with pseudo-3D bars rising beside each country to show renewable output alongside the price colouring. She wrote this process book.

**Hajj Hannah** worked on the analytical side. She ran the early exploratory data analysis that produced the findings the piece is built around (the negative-hour distribution across the five zones, the Switzerland to Germany co-occurrence rate, and the year-over-year duck-curve shift) and helped compose the Step 7 small multiples.
